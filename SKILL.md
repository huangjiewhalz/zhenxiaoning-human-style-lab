...
6. 需要视觉风格、连续配图或布局避重时，读取 `references/layout-selection-engine.md`、`references/presentation-carriers.md` 和 `references/visual-ip-system.md` 的构图规则，生成 `composition-patterns.md`。
...
10. 用户要求把当前窗口、项目过程、对话迭代或 AI 实验过程做成图文复盘时，读取 `references/window-experiment-log.md`，先生成实验卡、布局规划表和中文短词白名单；有生图能力时再生成图片，没有生图能力时输出可复制 prompt。
...
- 视觉输出先自动判断密度档位、构图尺度和留白偏好：轻配图、中密度解释图或高密度工作台；局部轻描、小场景、完整场景或工作台；自动、留白多、平衡或内容更丰富。不确定时优先轻配图和局部轻描；用户明确说“留白多一点”“更精致”“内容更丰富”“一张讲清楚”时，优先执行用户偏好。
- 密度差异必须是结构差异，不只是文字或纸片数量差异：轻配图是一个动作，高密度是一个系统；连续配图要轮换纸片、卡片、日历、地图、控制台、对话、空间剖面和真实道具等信息承载物。
- 呈现方式必须有载体选择：纸面文具只是其中一种；评论、产品、教程、复盘、证据和发布场景要优先从 `presentation-carriers.md` 选择基础或新颖载体。用户反馈“不够新颖”时，优先尝试微型街区、地铁换乘、暗房显影、收音调频、邮局分拣、裁缝试衣、厨房备料、博物馆展柜、舞台后台、天气观测、沙盘推演、小店货架、维修工位或观测星图。
