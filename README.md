...
- **2026-06-22**：明确仓库中的镇小宁形象是作者自有 IP 的展示案例，用于说明固定角色绑定效果；公开仓库不默认授权复用该角色，用户应绑定自己的原创或已授权角色。
- **2026-06-22**：新增三风格调度规则：白底彩铅负责解释观点和机制，抽象镇小宁负责状态 / 情绪 / 转场小图，虚实结合负责现实证据和工作现场。
- **2026-06-22**：收紧抽象镇小宁规则：抽象版本使用唯一母版，不从多张参考随机选择；每次变形先做 3-5 个身体系统候选，再按主题贴合度选择。
- **2026-06-22**：禁止本地排版补字：图中文字由生图模型自然画成短中文批注，关键文字失败时减少短词或放到图外说明，不用 Pillow、ImageMagick 或截图叠字。
- **2026-06-15**：新增可选“抽象镇小宁涂鸦风”：把镇小宁处理成更随手、更抽象、更有人味和艺术感的彩铅乱线变体；“像真人”只指情绪和姿态，不画真人肖像。
- **2026-06-15**：抽象镇小宁支持固定母版：一旦本机或用户资产里绑定了唯一抽象母版，后续生成必须从母版延展，不再每次从文字重新发明抽象画风。
- **2026-06-15**：新增可选“实物拼贴手绘风”：用真实物件搭小工作现场，再让手绘角色参与拉线、贴标签、按住、修补等动作，适合 Skill 展示、工作流复盘和 AI 实验日志。
- **2026-06-15**：调整固定角色默认尺度：固定角色在常规正文配图、通用展示和方向预览中默认作为中等小角色参与动作，不再占据过大的主角面积。
...
镇小宁是本项目作者自有 IP 的展示案例，用来说明“固定角色 + 内容文案 + 风格规则”可以如何形成稳定配图系统。

公开仓库默认不内置、不授权复用镇小宁角色。请使用自己的原创角色、已授权角色，或通过本机不提交的 `references/ip-subject.local.md` 绑定私有角色。如果想使用镇小宁形象，请先联系作者获得授权。
